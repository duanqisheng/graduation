...
file = open('F:/毕业设计/ass_ticker_number/'+str(factor_name) + '.csv')
...
for j in range(731):
    print(j)
    group = pd.Series(data.sort_values(by=[str(j)])[str(j)][59*5-1:59*5]).index.tolist()
    print(group)
    # 计算等权高配组合
    profit_list.append(graduation.Calculate_profit.group_profit_for_dayi(group,j))
df = pd.DataFrame(profit_list)
# df.to_csv('F:/毕业设计/ass_ticker_number/ass_profit_1.csv')
file.close()
