...
for i in range(9):
    ...
    if i == 8:
        ...
        for k in range(732):
            for j in range(295):
                data.at[graduation.Ticker.List[order[k][j]],k] = j # 升序，因子值大，分数大
        data.to_csv('F:/毕业设计/ass_ticker_number/'+str(factor_name) + '.csv')
    else:
        factor_name = graduation.Create_final_factor_data.name_list[i]
        print(factor_name)
        df = graduation.Create_final_factor_data.d[factor_name].df
        ...
        for k in range(732):
            for j in range(295):
                data.at[graduation.Ticker.List[order[k][j]],k] = abs(j-295) # 因子值大，分数小
        data.to_csv('F:/毕业设计/ass_ticker_number/'+str(factor_name) + '.csv')
