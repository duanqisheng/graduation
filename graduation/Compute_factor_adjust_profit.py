import graduation.Create_factor_adjust_ass_data
...
for factor in graduation.Factor_name.List:
    print(factor)
    factor_wight_df = graduation.Create_factor_adjust_ass_data.d[factor].df
    one_factor_profit = []
    for i in range(295):
        my_list = []
        for j in range(731):
            my_list.append(df.iloc[i,j+1] * factor_wight_df.iloc[j,i])
...
result.to_csv('F:/毕业设计/factor_ass_adjust_profit.csv')
